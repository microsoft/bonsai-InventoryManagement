...
import pdb
from or_gym.envs.supply_chain.multi_sku import SKUInfoFactoryRandom
...
def make_multi_sku_env(config:dict= {'number_of_stages':4, 'number_of_sku':1})->InvManagementLostSalesMultiSKUEnv:
    ...
    skus = SKUInfoFactoryRandom(sku_count = config["number_of_sku"], topography = my_chain, config = config)
    print(skus.info)
    #time.sleep(10)
    ...
    def episode_step(self, action: Dict[str, Any]):
        ...
        for i in range(0, self.action_frequency):
            ...
            if None in [item for sublist in sim_action for item in sublist]:
                self.mip_cost_config = "Hybrid"
                sim_action, _, _ = self.mip_solver.get_mip_action_multiple_sku(skus = skus, cost_function="Hybrid", constraint_coupling = bool(0))
        ...
        while not terminal:
            ...
            for sku in range(sim.simulator.skus.sku_count):
                ...
                for level in range(sim.simulator.skus.n_levels):
                    transit_orders.append(list(sim.simulator.skus.info.dynamic[sku].transit_order[level])[0:15])
                transit_orders  = [item for sublist in transit_orders for item in sublist]   
                # make sure this matches that of platform 
                sim.simulator.skus.info.dynamic[sku].forecast_window = 15                
                sim_state = {}
                sim_state['transit_orders'] = [int(order) for order in transit_orders]
                sim_state['demand_actual'] = int(sim.simulator.skus.info.dynamic[sku].iteration_actual_demand) #    ["demand_actual"][sim.simulator.period].tolist()
                sim_state['demand_forecast']  = [int(i) for i in sim.simulator.skus.info.dynamic[sku].iteration_demand_forecast] # ["demand_forecast"][sim.simulator.period+1:sim.simulator.period+11].tolist()   
                sim_state['demand_sigma']  = [int(i) for i in sim.simulator.skus.info.dynamic[sku].iteration_forecast_sigma] # [sim.simulator.period+1:sim.simulator.period+11].tolist()   
                sim_state["inventory"] = [int(i) for i in sim.simulator.skus.info.dynamic[sku].inventory]
                sim_state['leads'] = [int(i) for i in sim.simulator.skus.info.dynamic[sku].leads]
                sim_state["constraint_relaxation"] = int(sim.constraint_relaxation)
                sim_state["mip_solver_status"] = sim.mip_cost_config 
                sim_state["sim_reward"] = float(sim.sum_cost_action_freq)        
                sim_state["missed_sale_to_inventory_cost_ratio"] = float(sim.simulator.skus.info.generic[sku].missed_sale_cost[0]/sim.simulator.skus.info.generic[sku].storage_cost[0])
                ...
                states_log["skus_missed_sale_to_inventory_cost_ratio"].append(int(sim.simulator.skus.info.generic[sku].missed_sale_cost[0]/sim.simulator.skus.info.generic[sku].storage_cost[0]))
            ...
            states_log["constraint_relaxation"] = int(sim.constraint_relaxation)
