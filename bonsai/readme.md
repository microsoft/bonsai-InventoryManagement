...
To train brain for specif cost ratio, use the following inkling file and modify "cost_ratio" constant near top of the file. The default is cost_ratio = 100. The results shown in the following sections are based on this inkling file. 
```
MachineTeacher_const_ratio.ink
'''
In order to train for different cost ratio using single brain training, you may use the following inkling. 
```
MachineTeacher_cost_ratio_range.ink
```
