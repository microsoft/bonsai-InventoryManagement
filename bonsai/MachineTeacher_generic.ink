...
const cost_ratio: number = 10 #missed sales to inventory hold cost 
...
type SimConfig {
    ...
    lead_profile: number<1 .. 3 step 1>, # 1 fixed, 2,3 variable 
    missed_sale_to_inventory_cost_ratio: number<1 .. 100>,
...
simulator orgym_sim(action: SimAction, config: SimConfig): SimState {
    # package with short lead time : 2, 3, 4 
    #package "ims03102022"
}
...
graph (input: ObservableState) {
    concept OptimLevel(input): SimAction {
        curriculum {
            ...
            lesson `demand profile1`{
                scenario {
                    lead_profile: 1, # 1: constant default, 2-3: stepwise variable leadtime 
                    # the follwing wil only be used for the profile = 5. Other profiles 1-4 and 6-9 are handled by the simulator  
            ...
            lesson `demand profile2`{
                scenario {
                    lead_profile: number<1 ..2 step 1 >, # 1: constant default, 2-3: stepwise variable leadtime 
                    # the follwing wil only be used for the profile = 5. Other profiles 1-4 and 6-9 are handled by the simulator  
            ...
            lesson `demand profile3`{
                scenario {
                    lead_profile: number <1 .. 3 step 1>, # 1: constant default, 2-3: stepwise variable leadtime 
