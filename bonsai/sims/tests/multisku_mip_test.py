...
def test_solver_run_all_cost_functions() ->None:
    ...
    solver.get_mip_action_multiple_sku(skus=test_skus, cost_function="SafetyAddedHybrid")  
    
def test_solver_run_specific(Solver: MipSolver) -> None:
    import pickle
    with open('FailedSkuObject1.pickle', 'rb') as handle:
        skus = pickle.load(handle)
    PO1,_,_ = Solver.get_mip_action_multiple_sku(skus=skus, cost_function="Hybrid")
    PO2,_,_ = Solver.get_mip_action_multiple_sku(skus=skus, cost_function="SafetyAddedHybrid")  
...
# import pickle 
# with open('FailedSkuObject.pickle', 'rb') as handle:
#     skus = pickle.load(handle)
