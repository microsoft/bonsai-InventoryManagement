...
from or_gym.envs.supply_chain.multi_sku import SupplyChainTopography
...
class MipSolver:
    ...
    def get_mip_action_multiple_sku (self, skus: SKUInfoFactory, ratio: float = 0.1, cost_function: str = "Hybrid", constraint_coupling: bool = 1):
        ...
        if constraint_coupling == 1:
            ...
            for i in self.time:
                for j in self.level: 
                    if i>leads[j]:
                        model += xsum(IH[s][i][j] for s in self.sku) + xsum(R[s][i][j] for s in self.sku) <= skus.constraints.inventory_capacity[j] 
            print('imposing inventory constraint:', skus.constraints.inventory_capacity[j]) 
            # import time
            # time.sleep(3)       
        ...
        for s in self.sku:
            ...
            for i in self.time:
                for j in self.level:
                    if i >= leads[j]:
                        if j<2:
                            model += R[s][i][j] == P[s][i-leads[j]][j] - MS[s][i-leads[j]][j+1]
                        else: 
                ...
                for i in self.time:
                    for j in self.level:
                        if i==0:
                            if j == 0:
                                model += inventory_holding[j] - IH[s][i][j] + R[s][i][j] + MS[s][i][j] == D[i]
                                model += MS[s][i][j] >= D[i]-inventory_holding[j] - R[s][i][j] 
                                model += IH[s][i][j] >= -(D[i]-inventory_holding[j] - R[s][i][j])
                                model += IH[s][i][j] >= 0 
                            else: 
                                model += inventory_holding[j] - IH[s][i][j] + R[s][i][j] + MS[s][i][j] == P[s][i][j-1] 
                                model += MS[s][i][j]>= P[s][i][j-1] - inventory_holding[j] - R[s][i][j]
                                model += IH[s][i][j]>= -(P[s][i][j-1] - inventory_holding[j] - R[s][i][j])
                                model += IH[s][i][j] >= 0 

                        else:              
                            if j == 0:
                                model += IH[s][i-1][j] - IH[s][i][j] + R[s][i][j] + MS[s][i][j] == D[i]
                                model += MS[s][i][j] >= D[i]-IH[s][i-1][j] - R[s][i][j] 
                                model += IH[s][i][j] >= -(D[i]-IH[s][i-1][j] - R[s][i][j]) 
                                model += IH[s][i][j] >= 0 

                            else: 
                                model += IH[s][i-1][j] - IH[s][i][j] + R[s][i][j] + MS[s][i][j] == P[s][i][j-1] 
                                model += MS[s][i][j]>= P[s][i][j-1] - IH[s][i-1][j] - R[s][i][j]
                                model += IH[s][i][j]>= -(P[s][i][j-1] - IH[s][i-1][j] - R[s][i][j])                
                                model += IH[s][i][j] >= 0 
                        if self.cost_function =="Safety":
        ...
        try:    
            print('sum of inventory at time 3 at level 1:', xsum(IH[s][3][1] for s in self.sku).x + xsum(R[s][3][1] for s in self.sku).x)   
        except:
    ...
    def get_mip_action_multiple_sku_v2 (self, skus: SKUInfoFactory, ratio: float = 0.1, cost_function: str = "Safety", constraint_coupling: bool = 1, relaxation = 0):
                    ...
                    for j in self.level:
                        ...
                        if i < leads[j] and i > 0:
                            if j == 0:
                                ...
                                # apply order of the previous level as demand
                                model += IH[s][i][j] >= (IH[s][i-1][j] + R[s][i][j] - P[s][i][j-1])
                                model += IH[s][i][j] >= 0
                        if i == 0:
                            if j == 0:
                                model += IH[s][i][j] >= (inventory_holding[j] +
        ...
        MS = np.empty((len(self.sku),len(self.time), len(self.level)), dtype = int)
        try:
            for s in self.sku:
                inventory_holding = skus.info.dynamic[s].inventory
                D = list(skus.info.dynamic[s].iteration_demand_forecast) 
...
if __name__ == "__main__":
    n_sku = 2
    n_stages = 4 
    test_chain = SupplyChainTopography(number_of_stages= n_stages)
    test_skus = SKUInfoFactory(sku_count=n_sku,topography=test_chain)
    test_skus.constraints.inventory_capacity = [50, 50, 50]
    my_solver = MipSolver(config = None)
    po,ms, ih = my_solver.get_mip_action_multiple_sku_v2(skus = test_skus , cost_function="Safety", constraint_coupling = 1, relaxation=1)
